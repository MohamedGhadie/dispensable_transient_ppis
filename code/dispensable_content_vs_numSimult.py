...
from perturbation_analysis import perturbed_ppis_max_num_simult
...
def main():
    ...
    naturalPerturbs ["perturbed_ppis_max_num_simult"] = naturalPerturbs.apply(lambda x:
                                                        perturbed_ppis_max_num_simult (x["protein"],
                                                                                       x["partners"],
                                                                                       x["perturbations"],
                                                                                       simultaneous), axis=1)
    diseasePerturbs ["perturbed_ppis_max_num_simult"] = diseasePerturbs.apply(lambda x:
                                                        perturbed_ppis_max_num_simult (x["protein"],
                                                                                       x["partners"],
                                                                                       x["perturbations"],
                                                                                       simultaneous), axis=1)
    ...
    for minDegree in degRange:
        if mono_edgetic:
            numNaturalMut_edgetic = sum((naturalPerturbs["mono-edgotype"] == 'mono-edgetic') &
                                        (naturalPerturbs["perturbed_ppis_max_num_simult"] >= minDegree))
            numDiseaseMut_edgetic = sum((diseasePerturbs["mono-edgotype"] == 'mono-edgetic') &
                                        (diseasePerturbs["perturbed_ppis_max_num_simult"] >= minDegree))
        else:
    ...
    for maxDegree in degRange:
        if mono_edgetic:
            numNaturalMut_edgetic = sum((naturalPerturbs["mono-edgotype"] == 'mono-edgetic') &
                                        (naturalPerturbs["perturbed_ppis_max_num_simult"] < maxDegree))
            numDiseaseMut_edgetic = sum((diseasePerturbs["mono-edgotype"] == 'mono-edgetic') &
                                        (diseasePerturbs["perturbed_ppis_max_num_simult"] < maxDegree))
        else:
            numNaturalMut_edgetic = sum((naturalPerturbs["edgotype"] == 'edgetic') & 
                                        (naturalPerturbs["perturbed_ppis_max_num_simult"] < maxDegree))
            numDiseaseMut_edgetic = sum((diseasePerturbs["edgotype"] == 'edgetic') & 
    ...
    curve_plot ([pN_E_low, pN_E_high],
                ...
                ewidth = 2,
                ecolors = ['k', 'r'],
                xlabel = 'Disrupted PPI number of simultaneous PPIs (c)',
                ylabel = 'Fraction of PPIs neutral upon disruption (%)',
                leg = ['Number of simultaneous PPIs < c', 'Number of simultaneous PPIs ≥ c'],
                ...
                show = showFigs,
                figdir = figDir,
                figname = 'Fraction_disp_PPIs_vs_numSimult%s_%.2f' % ('_monoedgetic' if mono_edgetic else '',
                                                                      overlap_cutoff))
