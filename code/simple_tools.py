...
def sequence_overlap (seq1, seq2):
    """Calculate the fraction of positions in a sequence that overlap with another sequence.
	
    Args:
        seq1 (array): positions in first sequence.
        seq2 (array): positions in second sequence.
    
    Returns:
        numeric
    
    """
    seq1_overlap = len([i for i in seq1 if i in seq2]) / len(seq1)
    seq2_overlap = len([i for i in seq2 if i in seq1]) / len(seq2)
    return seq1_overlap, seq2_overlap
